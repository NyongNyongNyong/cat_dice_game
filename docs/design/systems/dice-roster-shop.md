...
| 오퍼 | 면 | 가격 | 동작 |
|------|-----|------|------|
| `dice_triple_h` | H,H,H,1,1,1 | 2골드 | 최고 숫자면 |
| `dice_triple_l` | L,L,L,3,3,3 | 2골드 | 최저 숫자면 |
| `dice_triple_v` | V,V,V,2,2,2 | 3골드 | 보드에 없는 숫자 중 최저 |

- **동선:** 구매 주사위 클릭 → 교체 슬롯 클릭. 상세 [shop-purchase.md](shop-purchase.md).
...
| 상점 제목 | `상점` |
| 구매 행 | H / L / V 카드 — 미리보기 면, 이름, `N 골드` |
| 보유 행 | `슬롯 N` + 미리보기 면 + 이름 — 클릭으로 교체 대상 |
| 진행 버튼 | `다음 층` |
| 상태 문구 | 2단계 동선 안내 · 골드 부족 · 교체 완료 |
...
- [x] 상점에서 **골드 지불** 후 H/L/V **교체**가 로스터에 반영된다.
- [ ] 다음 층 시작 시 교체된 주사위로 굴림·점수·특수 면 표시가 동작한다.
